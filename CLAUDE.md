...
**Movies** (16 themes as of this writing — `oscarCinematography`/`rtHorror`/`rtWar`/`rtAnimation`/`palmeDor`/`oscarScreenplay`/`letterboxd500`/`oscarForeign`/`rtAction`/`oscarDirector`/`oscarVFX`/`oscarActor`/`oscarActress`/`doubanTvCn`/`doubanTvForeign`/`doubanTvAnime`):
...
**Douban TV themes** (`doubanTvCn`/`doubanTvForeign`/`doubanTvAnime` — 豆瓣9分华语剧集 / 国外剧集 / 动画; category tab 剧集): same movie pipeline, but the seed lists are scraped from Douban's own 选剧集 explore API rather than an editorial page — see `tools/douban-tv-seed/` (collector script + README). The seed order must be Douban's own default 综合排序 (`sort=T`) — the order a user sees scrolling the page. **Not `sort=S`** (高分优先): within 9~10 almost everything at 9.6+ is a per-season entry of a long-running anime/US series, so taking the top N by rating drops the whole 9.0~9.3 band and loses 《爱，死亡和机器人》/《鬼灭之刃》/《葫芦兄弟》 — first-screen entries on the real page (the first cut of these lists made exactly that mistake). Four API gotchas are documented there: only `tags` filters (`selected_categories` is silently ignored server-side), `score_range` rejects decimals, any single `tags` combo caps at 500 items, and "类型=全部剧集" has no tag of its own — it is the region's ranked list minus the `地区,综艺` pool (that exclusion pool must be complete, so it is gathered by partitioning on year tags). Seed rows carry `doubanId`, so `enrichThemeMovies` skips its Douban search entirely and pulls the detail directly — TV detail lives at `/rexxar/api/v2/tv/{id}`, which `/movie/{id}` 302s to and axios follows, and the search path's "non-movie subtype" quality gate doesn't apply to the manual-`doubanId` branch. `originalTitle` is deliberately set equal to `title`: these lists are ranked by *live* Douban rating, so ranks drift on every re-seed, and the title+year identity key is what keeps `_id` (and therefore users' marks) stable across re-runs.
