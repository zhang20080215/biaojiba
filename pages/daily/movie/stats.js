...
Page({
  ...
  onLoad() {
    ...
    wx.setNavigationBarTitle({ title: '电影统计' });
    this.fetchPeriod();
  },
  ...
  fetchPeriod() {
    ...
    wx.cloud.callFunction({
      name: 'syncDailyLog',
      data: { action: 'getRange', theme: 'movie', from: period.from, to: period.to },
      success: res => {
        const result = res && res.result;
        if (!result || !result.success) {
          toast.show(this, '加载失败');
  ...
  renderWeek(days, period) {
    ...
    const bars = days.map(day => {
      ...
      return this.barItem(wd, (day.entries || []).length, maxValue, flattenMovies([day])[0]);
    });
    const prefix = this.data.weekOffset === 0 ? '本周' : (this.data.weekOffset === -1 ? '上周' : `${-this.data.weekOffset}周前`);
    this.setData({
      headlineCap: `${prefix}观影`,
  ...
  renderMonth(days, period) {
    ...
    const bars = days.filter(d => (d.entries || []).length > 0)
      .map(day => this.barItem(String(Number(day.date.slice(8))), (day.entries || []).length, maxValue, this.topMovie(flattenMovies([day]))));
  ...
  renderYear(days) {
    ...
    flattenMovies(days).forEach(movie => {
    ...
    const thisYear = Number(this.data.today.slice(0, 4));
    this.setData({
      headlineCap: `${this.data.year}年观影`,
      headline: { num: stats.total, sub: `平均评分 ${stats.avgRatingText} · 活跃 ${stats.activeDays} 天` },
