...
Page({
  ...
  onLoad(options) {
    ...
    wx.setNavigationBarTitle({ title: '年度书单' });
    this.fetchYear();
  },
  ...
  renderYear(days) {
    const movies = flattenBooks(days).map(movie => ({
      ...movie,
    ...
    const favoriteAuthor = this.pickFavoriteAuthor(movies);
    ...
    this.setData({
      ...
      review: {
        ...
        topMovie,
        bestMonth: bestMonth && bestMonth.count ? `${bestMonth.month}月 · ${bestMonth.count}本` : '-',
        favoriteAuthor
      }
    });
  },
  ...
  pickFavoriteAuthor(movies) {
    const map = {};
    (movies || []).forEach(movie => {
      const author = (movie.author || '').split('/')[0].trim();
      if (!author) return;
      map[author] = (map[author] || 0) + 1;
    });
    const sorted = Object.keys(map).sort((a, b) => map[b] - map[a]);
    return sorted.length ? `${sorted[0]} · ${map[sorted[0]]}本` : '-';
  }
});
