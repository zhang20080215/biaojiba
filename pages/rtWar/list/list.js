...
Page({
    ...
    onShow() {
        const currentTheme = 'theme-rt-war';
        ...
        this.setData({ showSharePicker: false }, () => {
            wx.nextTick(() => {
                adManager.showInterstitial('share_interstitial').then(() => {
                    wx.navigateTo({
                        url: `/pages/rtWar/share/share?type=${type}`,
                        complete: () => {
    ...
    async loadAllMovies(forceRefresh = false) {
        ...
        try {
            ...
            const { movies, marks } = await DataLoader.loadMoviesData('rtWar', openid, forceRefresh, QUERY_OPTIONS);
    ...
    async loadUserMarks() {
        ...
        wx.showNavigationBarLoading();
        try {
            const { marks } = await DataLoader.loadMoviesData('rtWar', openid, false, QUERY_OPTIONS);
    ...
    onShareAppMessage() {
        return {
            title: '史上最佳战争电影 - 标记你的观影清单',
            path: '/pages/rtWar/list/list'
        };
    },
