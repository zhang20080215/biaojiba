...
Page({
    ...
    onShow() {
        const currentTheme = 'theme-palme';
        ...
        this.setData({ showSharePicker: false }, () => {
            wx.nextTick(() => {
                adManager.showInterstitial('share_interstitial').then(() => {
                    wx.navigateTo({
                        url: `/pages/palmeDor/share/share?type=${type}`,
                        complete: () => {
    ...
    async loadAllMovies(forceRefresh = false) {
        ...
        try {
            ...
            const { movies, marks } = await DataLoader.loadMoviesData('palmeDor', openid, forceRefresh, QUERY_OPTIONS);
    ...
    async loadUserMarks() {
        ...
        wx.showNavigationBarLoading();
        try {
            const { marks } = await DataLoader.loadMoviesData('palmeDor', openid, false, QUERY_OPTIONS);
    ...
    onShareAppMessage() {
        return {
            title: '历届金棕榈奖 - 标记你的观影清单',
            path: '/pages/palmeDor/list/list'
        };
    },
