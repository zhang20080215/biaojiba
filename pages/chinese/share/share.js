// pages/chinese/share/share.js - 华语高分电影海报生成页
const CanvasHelper = require('../../../utils/canvasHelper.js');
const DataLoader = require('../../../utils/dataLoader.js');
const ChineseLoader = require('../../../utils/chineseLoader.js');
const ChinesePosterDrawer = require('../../../utils/chinesePosterDrawer.js');
...
Page({
    ...
    async onLoad(options) {
        try {
            wx.setNavigationBarTitle({ title: '华语高分电影 TOP100 海报' });
    ...
    _setupCanvas(canvasNode, resolve, reject) {
        try {
            ...
            this.posterDrawer = new ChinesePosterDrawer(this.canvasHelper);
            resolve();
        } catch (err) {
    ...
    drawCardBackground() {
        ...
        gradient.addColorStop(0, '#fce4e4');
        gradient.addColorStop(1, '#fde8d4');
    ...
    drawMovieTagBackground(ctx, x, y, width, height, status) {
        ...
        const styles = {
            ...
            wish: { gradient: ['rgba(255, 193, 7, 0.2)', 'rgba(255, 193, 7, 0.12)'], stroke: 'rgba(255, 193, 7, 0.4)' },
    ...
    drawMovieTagText(ctx, x, y, width, height, title, status) {
        ...
        const colors = { watched: '#4CAF50', wish: '#FF8F00', unwatched: '#9E9E9E' };
    ...
    async exportAndSaveImage() {
        ...
        try {
            ...
            // 同步绘制结束后 setTimeout 不可靠（部分机型 500ms 不够，导出空白）
