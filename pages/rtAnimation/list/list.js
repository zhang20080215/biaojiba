...
// 走通用主题流水线（generic_theme_movies + getThemeMovies），rank 升序（第1名在前）
...
Page({
    ...
    onLoad() {
        ...
        const savedTheme = 'theme-rt-animation';
        this.setData({
            statusBarHeight: 0,
            headerPadTop: 0,
            menuBtnHeight: 0,
    ...
    onShow() {
        const currentTheme = 'theme-rt-animation';
        ...
        this.setData({ showSharePicker: false }, () => {
            wx.nextTick(() => {
                adManager.showInterstitial('share_interstitial').then(() => {
                    wx.navigateTo({
                        url: `/pages/rtAnimation/share/share?type=${type}`,
                        complete: () => {
    ...
    async loadAllMovies(forceRefresh = false) {
        ...
        try {
            ...
            const { movies, marks } = await DataLoader.loadMoviesData('rtAnimation', openid, forceRefresh, QUERY_OPTIONS);
    ...
    async loadUserMarks() {
        ...
        wx.showNavigationBarLoading();
        try {
            const { marks } = await DataLoader.loadMoviesData('rtAnimation', openid, false, QUERY_OPTIONS);
    ...
    onShareAppMessage() {
        return {
            title: '史上最佳动画电影 - 标记你的观影清单',
            path: '/pages/rtAnimation/list/list'
        };
    },
