// pages/rtAnimation/share/share.js
...
// 走通用主题流水线（generic_theme_movies + getThemeMovies）
const QUERY_OPTIONS = { orderByField: 'rank', orderDirection: 'asc' };

const TITLE = '史上最佳动画电影观影海报';
...
Page({
    ...
    async onLoad(options) {
        try {
            ...
            const themeClass = 'theme-rt-animation';
            this.setData({
    ...
    onBack() {
        wx.navigateBack({
            fail: () => {
                wx.reLaunch({ url: '/pages/rtAnimation/list/list' });
            }
        });
    },
    ...
    async loadData() {
        try {
            ...
            const { movies: allMovies, marks: allMarks } = await DataLoader.loadMoviesData('rtAnimation', openid, false, QUERY_OPTIONS);
