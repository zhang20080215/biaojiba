...
// 走通用主题流水线（generic_theme_movies + getThemeMovies），rank 升序（第1名/最新在前）
const QUERY_OPTIONS = { orderByField: 'rank', orderDirection: 'asc' };
...
Page({
    data: {
        ...
        tempAvatar: '',
        tempNickname: '',
        themeClass: 'theme-oscar-screenplay',
        statusBarHeight: 20,
        headerPadTop: 0,
        menuBtnHeight: 32,
        stickyTop: 0,
    ...
    onLoad() {
        ...
        const savedTheme = 'theme-oscar-screenplay';
        this.setData({
            statusBarHeight: 0,
            headerPadTop: 0,
            menuBtnHeight: 0,
        ...
        this.loadAllMovies(true); // 通用主题 listVersion 恒为 null，进页面强制拉最新
        this.initAds();
    ...
    onShow() {
        const currentTheme = 'theme-oscar-screenplay';
        ...
        this.setData({ showSharePicker: false }, () => {
            wx.nextTick(() => {
                adManager.showInterstitial('share_interstitial').then(() => {
                    wx.navigateTo({
                        url: `/pages/oscarScreenplay/share/share?type=${type}`,
                        complete: () => {
    ...
    async loadAllMovies(forceRefresh = false) {
        ...
        try {
            ...
            const { movies, marks } = await DataLoader.loadMoviesData('oscarScreenplay', openid, forceRefresh, QUERY_OPTIONS);
    ...
    async loadUserMarks() {
        ...
        wx.showNavigationBarLoading();
        try {
            const { marks } = await DataLoader.loadMoviesData('oscarScreenplay', openid, false, QUERY_OPTIONS);
    ...
    onShareAppMessage() {
        return {
            title: '历届奥斯卡最佳原创剧本 - 标记你的观影清单',
            path: '/pages/oscarScreenplay/list/list'
        };
    },
