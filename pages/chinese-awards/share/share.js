...
Page({
  ...
  async onReady() {
    try {
      ...
      wx.showModal({ title: 'Canvas 初始化失败', content: err.message || '请重试', showCancel: false });
    }
  },
  ...
  onBack() {
    wx.navigateBack({
      fail: () => {
        wx.reLaunch({ url: '/pages/chinese-awards/list/list' });
      }
    });
  },
  ...
  onBgThemeTap(e) {
    ...
    if (!theme) return;
    this.setData({ activeBgTheme: theme.key, currentGradient: { start: theme.start, end: theme.end } });
  },
  ...
  async loadData() {
    try {
      ...
      const { movies, marks } = await DataLoader.loadMoviesData('chinese_awards', openid, true);
      const allMovies = movies.map(movie => ({ ...movie, _id: String(movie._id) }));
      const { markStatusMap, stats, watchedMovies } = DataLoader.processMarks(marks, allMovies);
  ...
  getPosterLayoutConfig(totalInput) {
    ...
    const layouts = [];
    for (let cols = 1; cols <= maxCols; cols++) layouts.push(buildLayout(cols));
    ...
    if (chosen.canvasHeight > FIXED_CANVAS_HEIGHT && chosen.cols < maxCols) {
      ...
      if (denserLayout.canvasHeight < chosen.canvasHeight) chosen = denserLayout;
    }
    if (chosen.canvasHeight > MAX_POSTER_CANVAS_HEIGHT) chosen = { ...chosen, canvasHeight: MAX_POSTER_CANVAS_HEIGHT };
    return chosen;
  },
  ...
  async startDrawing() {
    let nextCanvasSize;
    if (this.data.shareType === 'poster') nextCanvasSize = { width: FIXED_CANVAS_WIDTH, height: this.getPosterLayoutConfig().canvasHeight };
    else if (this.data.textStyle === 'capsule') nextCanvasSize = { width: FIXED_CANVAS_WIDTH, height: Math.max(FIXED_CANVAS_HEIGHT, this.estimateCapsuleHeight()) };
    else nextCanvasSize = { width: FIXED_CANVAS_WIDTH, height: FIXED_CANVAS_HEIGHT };
    if (this.data.canvasSize.width !== nextCanvasSize.width || this.data.canvasSize.height !== nextCanvasSize.height) {
      this.resizeCanvas(nextCanvasSize.width, nextCanvasSize.height);
    }
    this.canvasHelper.clear();
    if (this.data.shareType === 'poster') await this.drawPosterWall();
    else await this.drawTextCard();
  },
  ...
  getPosterMetrics(card) {
    ...
    return { ...layout, startX, startY: card.contentY };
  },
  ...
  async drawPosterWall() {
    ...
    for (let i = 0; i < movies.length; i += batchSize) {
      ...
      if (total > 0) wx.showLoading({ title: `生成中 ${Math.floor(((i + batch.length) / total) * 100)}%`, mask: true });
    }
    if (!movies.length) {
      const ctx = this.canvasHelper.ctx;
      const palette = this.getPalette();
  ...
  async drawPosterItem(movie, x, y, width, height) {
    ...
    try {
      ...
      if (imageUrl.startsWith('cloud://')) imageUrl = await this.canvasHelper.getCloudTempUrl(imageUrl);
  ...
  drawPosterPlaceholder(x, y, width, height) {
    ...
    const palette = this.getPalette();
    ctx.save();
    ctx.fillStyle = 'rgba(248, 243, 231, 0.9)';
    this.canvasHelper.drawRoundRectPath(x, y, width, height, 8);
    ctx.fill();
    ctx.fillStyle = palette.textMuted;
  ...
  buildTagLayout(card) {
    ...
    const items = this.data.allMovies.map((movie, index) => {
      ...
      return { x: panelX + column * (cellWidth + gapX), y: panelY + paddingTop + row * rowHeight, width: cellWidth, height: rowHeight, fontSize, dotSize, textLeftGap, title: movie.title, status: this.data.markStatusMap[movie._id] || 'unwatched' };
    });
    ctx.restore();
  ...
  async drawTextCard() {
    const card = this.drawCard();
    this.drawCardHeader(card);
    if (this.data.textStyle === 'capsule') this.drawCapsuleLayout(card);
    else this.drawTextMatrix(this.buildTagLayout(card));
    this.drawFooter(card);
  },
  ...
  drawCapsuleLayout(card) {
    ...
    const colorMap = { watched: '#6F8244', wish: '#A07F12', unwatched: '#6F6F68' };
    const borderMap = { watched: 'rgba(154, 171, 101, 0.35)', wish: 'rgba(212, 168, 40, 0.35)', unwatched: 'rgba(156, 153, 143, 0.28)' };
    ctx.save();
    ctx.font = `600 ${fontSize}px sans-serif`;
    movies.forEach(movie => {
  ...
  drawTextMatrix(layout) {
    ...
    layout.items.forEach(item => this.drawMovieText(item));
  },

  drawMovieText(item) {
    const ctx = this.canvasHelper.ctx;
    const palette = this.getPalette();
    const dotColor = { watched: palette.watched, wish: palette.wish, unwatched: palette.unwatched }[item.status] || palette.unwatched;
      ...
      wx.authorize({
        ...
        fail: (err) => {
          if (err.errMsg && err.errMsg.includes('auth deny')) {
            wx.showModal({ title: '需要授权', content: '请在设置中允许访问相册', confirmText: '去设置', success: (res) => { if (res.confirm) wx.openSetting(); } });
  ...
  initAds() {
    if (this.data.adUnitIds.share_banner) this.setData({ showBannerAd: true });
  },
  ...
  onShareAppMessage() {
    return { title: '我的华语电影最高荣誉殿堂观影海报', path: '/pages/chinese-awards/list/list' };
  },
