// pages/oscarAnime/share/share.js - Oscar 最佳动画长篇 海报生成页
...
Page({
    ...
    async onLoad(options) {
        try {
            wx.setNavigationBarTitle({ title: '奥斯卡最佳动画长篇海报' });
    ...
    async loadData() {
        try {
            ...
            const { movies, marks } = await DataLoader.loadMoviesData('oscarAnime', openid, false);
    ...
    async drawMovieWall() {
        ...
        // 公式由「可用区宽高比 × 目标海报比 1.4」推导：奥斯卡(97)→11，最佳动画(25)→6。
    ...
    drawCanvasHeader(ctx, width, startY) {
        ...
        ctx.fillText('Academy Awards · Best Animated Feature', width / 2, startY + 34);
