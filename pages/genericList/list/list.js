...
Page({
    ...
    onMarkSheetPick(e) {
        const status = e.currentTarget.dataset.status || '';   // 'watched' | 'wish' | ''(没看过)
    ...
    // 统一标记入口：targetStatus 为 'watched'|'wish'|''（''=没看过=取消标记）。乐观更新 + 写 Marks
    setMark(movieId, targetStatus) {
        ...
        if (currentStatus === targetStatus) return;   // 无变化
