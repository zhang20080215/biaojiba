// pages/genericList/share/share.js —— 通用榜单海报生成页（?theme=xxx 决定主题）
...
// 排序方向按主题不同，见 utils/genericThemeConfig.js 的 cfg.orderDirection
...
Page({
    ...
    async onLoad(options) {
        try {
            ...
            this.setData({
                ...
                theme,
                cfg,
                'uiText.cardTitle': `${cfg.title}观影海报`
            });
    ...
    onBack() {
        wx.navigateBack({
            fail: () => {
                wx.reLaunch({ url: `/pages/genericList/list/list?theme=${this.data.theme}` });
            }
        });
    },
    ...
    async loadData() {
        try {
            ...
            const { movies: allMovies, marks: allMarks } = await DataLoader.loadMoviesData(this.data.theme, openid, false, queryOptionsFor(this.data.cfg));
    ...
    // 不随画布宽度等比缩放——缩小画布本身+放大字号要同时发生，两者若绑一起等比缩就互相抵消了）。
    // 找不到（即使 BASE_FONT 都放不下）时返回 null，调用方应换更大的画布尺寸再试。
    searchCapsuleFontForCanvas(canvasWidth, canvasHeight, baseFont, maxFont) {
        const scale = canvasWidth / FIXED_CANVAS_WIDTH; // 只用来推头部/边距等"画框"元素的比例，不用于字号本身
        ...
        if (baseHeight > targetHeight) return null; // 连最小字号都放不下这个画布
    ...
    // 决定胶囊文字卡片用哪个画布尺寸 + 字号：优先试紧凑画布（1080×1440，画布更小+字号可以
    // 更大，两头一起挤，明显比只放大字号丰满）；紧凑画布连基准字号都放不下时，退回标准画布
    // （1242×1660）重新找字号；标准画布基准字号都放不下（电影很多）时，维持原有的动态增高
    ...
    buildTagLayout(card) {
        ...
        // 字号是 startDrawing 阶段 decideListLayout 已经算好、连同画布尺寸一起决定的绝对值；
        // 行高不再无脑拿面板高度除以行数——那样电影少、行数少时会把每行拉得很开（行间距过大）。
        // 多出来的空间整体垂直居中；行数多到舒适行高放不下时，才收缩回"撑满面板"的算法兜底。
    ...
    drawMovieText(item) {
        ...
        // title text in dark color (matches preview)
    ...
    onShareAppMessage() {
        return {
            title: `我的${this.data.cfg.title}观影海报`,
