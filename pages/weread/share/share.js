...
Page({
    ...
    async loadData() {
        try {
            ...
            const allBooks = (rawBooks || []).map(b => ({
                ...b,
                _id: String(b._id),
                _displayTitle: simplifyTitle(b.title)  // 海报用，不入库
            }));
            const { markStatusMap, stats, readBooks } = DataLoader.processBookMarks(marks || [], allBooks);
    ...
    async startDrawing() {
        ...
        if (this.data.shareType === 'poster') {
            ...
            // 文字海报固定 1242×1660；capsule 内部按内容自适应缩小字号/胶囊填满
    ...
    drawCardHeader(card) {
        ...
        ctx.font = `700 ${Math.round(48 * scale)}px sans-serif`;
    ...
    buildTagLayout(card) {
        ...
        const items = this.data.allBooks.map((book, index) => {
            ...
            return {
                ...
                dotSize,
                textLeftGap,
                title: book._displayTitle || book.title,  // 海报用简化标题（去括号）
    ...
    drawCapsuleLayout(card) {
        ...
        // 留出 60rpx 缓冲避免 pill 与底部水印重叠（水印在 footerY 居中，font 20*scale）
        const maxHeight = card.footerY - card.contentY - Math.round(60 * scale);

        // 自适应缩放：把所有 pills 压进 (maxWidth, maxHeight) 内（默认尺寸塞不下时收缩）
        const measureLayoutHeight = (fit) => {
            const fs = 20 * scale * fit;
            const ph = 38 * scale * fit;
            const ppx = 16 * scale * fit;
            const gx = 10 * scale * fit;
            const gy = 6 * scale * fit;
            ctx.save();
            ctx.font = `600 ${Math.max(8, Math.round(fs))}px sans-serif`;
            let cx = 0;
            let cy = 0;
            for (const book of books) {
                const t = book._displayTitle || book.title;
                const tw = ctx.measureText(t).width;
                const pw = tw + ppx * 2;
                if (cx + pw > maxWidth && cx > 0) {
                    cx = 0;
                    cy += ph + gy;
                }
                cx += pw + gx;
        ...
        let fitScale = 1.0;
        if (measureLayoutHeight(1.0) > maxHeight) {
            let lo = 0.3;
            let hi = 1.0;
            for (let i = 0; i < 10; i++) {
                const mid = (lo + hi) / 2;
                if (measureLayoutHeight(mid) <= maxHeight) lo = mid;
                else hi = mid;
            }
            fitScale = lo;
        }

        const fontSize = Math.max(10, Math.round(20 * scale * fitScale));
        const pillH = Math.max(20, Math.round(38 * scale * fitScale));
        const pillPadX = Math.max(6, Math.round(16 * scale * fitScale));
        const gapX = Math.max(4, Math.round(10 * scale * fitScale));
        const gapY = Math.max(2, Math.round(6 * scale * fitScale));
        let curX = startX;
        ...
        books.forEach(book => {
            ...
            const displayTitle = book._displayTitle || book.title;  // 海报用简化标题
            const textW = ctx.measureText(displayTitle).width;
            ...
            // strikethrough for read
            if (status === 'read') {
                ctx.strokeStyle = 'rgba(111, 130, 68, 0.75)';
                ctx.lineWidth = Math.max(1, Math.round(1.5 * scale * fitScale));
                ctx.beginPath();
