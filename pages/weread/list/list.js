// 微信读书 TOP200 总榜列表页 — 镜像 pages/doubanBooks/list/list.js
// 差异：theme='weread_books'，BookMarks 集合（source='weread'），bookId 字段。
// 视觉：强制微信读书品牌绿 #3B9F4D，不随全局 app 主题切换。
...
Page({
    ...
    setNavBarColor() {
        // 微信读书强制品牌绿，不随全局 app 主题切换
        wx.setNavigationBarColor({ frontColor: '#ffffff', backgroundColor: '#3B9F4D', animation: { duration: 0 } });
    },
        ...
        this.setData({ showSharePicker: false }, () => {
            wx.nextTick(() => {
                adManager.showInterstitial('share_interstitial').then(() => {
                    wx.navigateTo({
                        url: `/pages/weread/share/share?type=${type}`,
                        complete: () => {
    ...
    async loadAllBooks(forceRefresh = false) {
        ...
        try {
            ...
            const { movies: rawBooks, marks } = await DataLoader.loadMoviesData('weread_books', openid, forceRefresh);
    ...
    async loadUserMarks() {
        ...
        wx.showNavigationBarLoading();
        try {
            const { marks } = await DataLoader.loadMoviesData('weread_books', openid, false);
    ...
    setMark(bookId, targetStatus) {
        ...
        if (!targetStatus) {
            ...
            const persist = db.collection('BookMarks').where({ bookId, openid }).remove();
            persist.catch(err => {
    ...
    batchUpdateBookMarks(bookIds, status) {
        ...
        wx.cloud.callFunction({
            ...
            data: { bookIds, status, openid, source: 'weread' },
            success: res => {
                wx.hideLoading();
                if (res.result && res.result.success) {
                    ...
                    // 本地状态没跟着改，用户看到的仍是旧的。重新拉一次标记，让显示收敛到云端真实状态。
                    setTimeout(() => { this.loadUserMarks(); }, 300);
                }
            },
            fail: err => {
