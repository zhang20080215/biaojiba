...
Page({
    ...
    onLoad() {
        ...
        const savedTheme = 'theme-rt-horror';
        this.setData({
            statusBarHeight: 0,
            headerPadTop: 0,
            menuBtnHeight: 0,
    ...
    onShow() {
        const currentTheme = 'theme-rt-horror';
        ...
        this.setData({ showSharePicker: false }, () => {
            wx.nextTick(() => {
                adManager.showInterstitial('share_interstitial').then(() => {
                    wx.navigateTo({
                        url: `/pages/rtHorror/share/share?type=${type}`,
                        complete: () => {
    ...
    async loadAllMovies(forceRefresh = false) {
        ...
        try {
            ...
            const { movies, marks } = await DataLoader.loadMoviesData('rtHorror', openid, forceRefresh, QUERY_OPTIONS);
    ...
    async loadUserMarks() {
        ...
        wx.showNavigationBarLoading();
        try {
            const { marks } = await DataLoader.loadMoviesData('rtHorror', openid, false, QUERY_OPTIONS);
    ...
    onShareAppMessage() {
        return {
            title: '史上最佳恐怖电影 - 标记你的观影清单',
            path: '/pages/rtHorror/list/list'
        };
    },
