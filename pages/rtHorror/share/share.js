...
Page({
    ...
    async onLoad(options) {
        try {
            ...
            const themeClass = 'theme-rt-horror';
            this.setData({
    ...
    onBack() {
        wx.navigateBack({
            fail: () => {
                wx.reLaunch({ url: '/pages/rtHorror/list/list' });
            }
        });
    },
    ...
    async loadData() {
        try {
            ...
            const { movies: allMovies, marks: allMarks } = await DataLoader.loadMoviesData('rtHorror', openid, false, QUERY_OPTIONS);
