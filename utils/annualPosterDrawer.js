// utils/annualPosterDrawer.js - 年度院线电影海报绘制器

const TITLE = '2026 年度院线电影观影海报墙';

function AnnualPosterDrawer(canvasHelper) {
    this.helper = canvasHelper;
    this.ctx = canvasHelper.ctx;
}

AnnualPosterDrawer.prototype.getTitle = function () {
    return TITLE;
};

AnnualPosterDrawer.prototype.drawPosterWall = async function (movies, canvasSize, updateProgress) {
...
AnnualPosterDrawer.prototype.drawSinglePoster = async function (movie, x, y, width, height) {
    const ctx = this.ctx;
...
AnnualPosterDrawer.prototype._drawPlaceholder = function (title, x, y, width, height) {
    const ctx = this.ctx;
...
module.exports = AnnualPosterDrawer;
