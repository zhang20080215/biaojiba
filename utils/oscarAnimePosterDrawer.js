// utils/oscarAnimePosterDrawer.js - Oscar 最佳动画长篇 海报绘制器

const TITLE = '历届奥斯卡最佳动画长篇观影海报墙';

function OscarAnimePosterDrawer(canvasHelper) {
    this.helper = canvasHelper;
    this.ctx = canvasHelper.ctx;
}

OscarAnimePosterDrawer.prototype.getTitle = function () {
    return TITLE;
};

OscarAnimePosterDrawer.prototype.drawPosterWall = async function (movies, canvasSize, updateProgress) {
...
OscarAnimePosterDrawer.prototype.drawSinglePoster = async function (movie, x, y, width, height) {
    const ctx = this.ctx;
...
OscarAnimePosterDrawer.prototype._drawPlaceholder = function (title, x, y, width, height) {
    const ctx = this.ctx;
...
module.exports = OscarAnimePosterDrawer;
