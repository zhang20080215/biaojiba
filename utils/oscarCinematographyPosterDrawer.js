// utils/oscarCinematographyPosterDrawer.js - Oscar 最佳摄影奖 海报绘制器

const TITLE = '历届奥斯卡最佳摄影奖观影海报墙';

function OscarCinematographyPosterDrawer(canvasHelper) {
    this.helper = canvasHelper;
    this.ctx = canvasHelper.ctx;
}

OscarCinematographyPosterDrawer.prototype.getTitle = function () {
    return TITLE;
};

OscarCinematographyPosterDrawer.prototype.drawPosterWall = async function (movies, canvasSize, updateProgress) {
...
OscarCinematographyPosterDrawer.prototype.drawSinglePoster = async function (movie, x, y, width, height) {
    const ctx = this.ctx;
    ...
    try {
        // 优先使用云存储的 cover 字段（已上传到云端），避免从豆瓣下载
...
OscarCinematographyPosterDrawer.prototype._drawPlaceholder = function (title, x, y, width, height) {
    const ctx = this.ctx;
...
module.exports = OscarCinematographyPosterDrawer;
