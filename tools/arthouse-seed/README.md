...
**豆瓣搜索搜不到的条目**：首轮灌库 250 条只进去 247 条，缺的是 **rank 128《蓝风筝》/ 180《春夏秋冬又一春》/ 189《颐和园》**——跟译名无关，是这些条目在豆瓣**搜索结果里不出现**（页面用 URL 直接打得开），`enrichThemeMovies` 的搜索路径拿不到候选就跳过了。本项目在 `fetchMovieFullInfo` 那边也踩过同类坑（豆瓣登录墙屏蔽的正片，当时用时光网补候选解决）。

补法是绕开搜索直接给 `doubanId`：填好 `arthouse.missing3.params.json` 里的三个 id 再灌一轮即可（带 `doubanId` 的条目跳过搜索直接取详情）。找 id 的办法：豆瓣搜**导演**进影人页翻作品列表，或用搜索引擎搜「豆瓣 片名 导演」直接命中条目页。重灌这份名单时这三条大概率还会掉，把 id 固化进 `arthouse.json` 就一劳永逸。
