...
- **《蓝风筝》`1303967`、《春夏秋冬又一春》`1316580`、《小城之春》`1323746`** 已固化进 `arthouse.json`（带 `doubanId` 的条目跳过搜索直接取详情）。

  ⚠ 但前两条**未必救得回来**：本地实测这两个 id 的 rexxar 详情接口返回 `need_permission`（而《小城之春》同一时刻正常返回），说明豆瓣对它们是**条目级**限制——既屏蔽搜索、也限制 API 详情。手动 `doubanId` 分支走的同样是 `fetchDoubanDetail`，云函数那边如果也拿到 `need_permission`，这两条照样进不去，会落在 `unmatchedMovies` 里报「详情请求失败」。云函数是腾讯云 IP，策略未必相同，重跑一次即可验证。真进不去就只能换片。

  《小城之春》的 id 是 **1948 费穆版**（已核对：8.7 分 / 7.4 万人评；2002 田壮壮翻拍是另一条目，靠搜索匹配有选错风险，所以钉死）。
- **《颐和园》豆瓣根本没有条目**，无法收录，换成了 **《小城之春》(1948, 费穆)**——顺带补上名单原先「1949 年前的中国电影一部没有」的空白。

找这类 id 的办法：豆瓣搜**导演**进影人页翻作品列表，或用搜索引擎搜「豆瓣 片名 导演」直接命中条目页。
