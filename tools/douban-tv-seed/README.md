# 豆瓣高分剧集三主题 灌库说明（doubanTvCn / doubanTvForeign / doubanTvAnime）
...
|---|---|---|---|---|
| `doubanTvCn` | 豆瓣高分华语剧集 | 类型=电视剧，地区=华语，**评分 8~10** | `电视剧,华语` | 综合排序前 250（池 500 上限）|
...
### 1. 华语的评分门槛是 8 分，标题也因此不写「9分」

`电视剧+华语+9~10` 的真实全量只有 **168 条**，凑不满 250，所以华语单独放宽到 `8~10`。
代价是前 250 里约 185 条低于 9 分（最低 8.0），因此这个主题的前端标题是**「豆瓣高分华语剧集」**，
不是「豆瓣9分…」。哪天改回 9 分门槛，记得把 `utils/genericThemeConfig.js` 和
`pages/category/category.js` 里的标题/描述一起改回去。国外和动画仍是 `9~10`。
...
- 单个 tags 组合**最多返回 500 条**（服务端硬上限）。三份都是 500，取前 250 够用；
...
灌完 `getThemeMovies` 分别测 `{ "theme": "doubanTvCn" }` 等，`movies` 长度应各为 250。
